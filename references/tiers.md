...
| Self-test | `scripts/harness_selftest.py` **copied into the project** (skill-relative paths rot in CI). At tier S it verifies manifest, components, markers, placeholder-freedom (installed docs *and* the manifest file), tier-pruning, and the rules hash; hooks add plant tests at tier M. | copy from `scripts/harness_selftest.py` |
...
| Status-guard hook | Blocks hand-editing a work item into a gated status when the edit carries no real evidence (empty-quoted template defaults don't count). | `scripts/hooks/status_guard.py` |
| Secret-scan hook | Blocks writes containing credential-shaped literals (known prefixes, high-entropy assignments). Skips test fixtures; env-var reads are fine. Known-prefix matches are high-confidence and fire even on an otherwise env-sanctioned line. | `scripts/hooks/secret_scan.py` |
...
| Harness self-test (extended) | The project-local copy now also plants a fake violation per hook and requires rejection (including MultiEdit-shaped and high-entropy-assignment plants), verifies hook wiring three ways (installed-but-unwired, wired-but-missing, and `hooks.enabled`-but-absent), and regenerates each `generated_docs` entry to diff against the committed copy. **A guard that has never failed a fake must be presumed hollow.** | copy from `scripts/harness_selftest.py` |
...
Hook wiring: copy hooks into the project's `.claude/hooks/`, register via `assets/templates/settings-hooks.json` merged into the project's `.claude/settings.json` — **include only the hooks you actually installed**, and keep the `test ! -f || python3` guard form so a missing hook file allows edits instead of blocking every one. Configure `protected_paths` and `gated_statuses` in `.claude/harness.json`, and **list the installed hooks under `hooks.enabled`** — the self-test requires every enabled hook to be present, wired, and plant-passing, so a deleted hook whose `test ! -f` settings line still lingers FAILs instead of passing silently. Hooks are thin and **fail open** (a broken hook must never brick the session) — which is exactly why the self-test must prove they fire when healthy.
