...
Read `references/drift-audit.md` for the full procedure and report format. **Scale depth to the diff, not the phase count:** below the diff-size threshold in drift-audit.md's Cadence section, the audit is the check command + self-test + one paragraph in ACTIVE.md — no report file. Above it, the fixed seven-check spine — run all seven, report PASS / PARTIAL / FAIL per check:
